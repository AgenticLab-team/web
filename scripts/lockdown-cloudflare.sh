...
#
# ⚠️ **2026-08-14：这条限制已按站长要求在线上撤销了。**
#
# 现在 80/443 对所有来源开放（ufw 里一条 `allow proto tcp from any`）。
# 这个脚本没有删，因为它记着这件事该怎么做；但**跑一次就会把限制装回去**，
# 而那不是当前想要的状态。要撤销的话跑 scripts/open-http.sh。
#
# 撤销的代价写在下面那段起因里，一个字都没变：源站 IP 一旦泄露
# （历史 DNS、证书透明日志、一封从这台机器发出的邮件），攻击能直接
# 落到源站上，Cloudflare 那层挡不住 —— 因为它被绕过去了。
#
