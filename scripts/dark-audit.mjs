 ...
 * 分不出同一个类名的第三个和第五个。
 */
/* 探针住在真文件里 —— 它被反引号截断过三次，见那个文件顶上的说明 */
const probe = readFileSync(new URL("./lib/probe-colors.js", import.meta.url), "utf8");
...
const brightness = (css) => {
  ...
  // `color(srgb …)`（`color-mix` 的产物）分量是 0–1，不是 0–255
  const k = css.startsWith("color(") ? 255 : 1;
  return 0.2126 * +m[0] * k + 0.7152 * +m[1] * k + 0.0722 * +m[2] * k;
};
