...
「以某身份预览」的记录。存哈希不存令牌，可随时掐断。

`withheld` 是「他有、预览者没有、所以预览里没给」的权限点 JSON 数组 ——
存下来是为了事后能回答「他当时看到的那个视角准不准」。

预览态下所有写操作被拒（`requireWritableAdmin`），
覆盖情况由 `tests/preview-coverage.test.ts` 静态核对：
调了 requireAdmin 又做写操作的函数，一个都不许漏。
漏一个的后果是管理员以别人的身份写了数据，而审计记在被预览的人头上 ——
从那以后审计日志一条都不能信。

### matrix_snapshots

权限矩阵的版本快照。存的是**那次改动发生之前**的整张表。

为什么不靠审计日志回滚：日志里是 `版主/forum.view=granted` 这样给人读的
字符串，要靠它还原得把字符串解析回三态、还得假设中间没有别的改动插进来。
一次三十格的改动出问题时，没有人有耐心去核对三十行文本。

回滚时遍历**快照与当前的并集**，不是只遍历快照 ——
只看快照会漏掉快照之后新增的格子，结果是回滚报告成功而问题还在原地。
