...
export function stalledBinds(): StalledBind[] {
  ...
  const rows = db
     ...
     * 作废的要排掉。
     *
     * 它仍然 matchedAt 为空、仍在 24 小时内 —— 光看这两条的话
     * 点了「作废」之后那一行还留在原地，而人会以为按钮没生效，
     * 于是再点一次。一个看起来没反应的操作，比一个报错的操作更糟。
     */
    .where(
      and(
        isNull(bindCodes.matchedAt),
        gt(bindCodes.createdAt, now - DAY_MS),
        ne(bindCodes.status, "revoked"),
      ),
    )
    .orderBy(desc(bindCodes.createdAt))
    .all();

  const fresh = rows.filter((r) => isActionable(r.createdAt, now));
