 ...
 * 第 5 次终于进去了的人,还剩 4 条没匹配的记录 ——
 * 队列上写着「取了 4 次码」,而他**早就登录进去了**。
 *
...
/**
 * 从这段时间里的**全部**取码记录里，挑出真正像卡住的人。
 *
 * 注意入参是全部记录，不是「没匹配上的那些」——
 * 少了成功的那部分就没法把「后来成功了」的人排掉。
 */
export function groupStuck(rows: CodeRow[], threshold = STUCK_CODE_THRESHOLD): StuckApplicant[] {
  const byIp = new Map<string, CodeRow[]>();
  ...
  for (const [ip, all] of byIp) {
    // 这段时间里成功过一次，就不是卡住的人 —— 他已经进去了
    if (all.some((r) => r.matched)) continue;

    const list = all.filter((r) => !r.matched);
