...
export const SURFACES: readonly Surface[] = [
  ...
  },
  {
    ...
    board: "me",
    /*
     * 网页上这是信里的一个下载链接，不是独立页面。
     */
    web: null,
    /*
     * ═════════════════════════════════════════
     * 终端里**还没做**，而这条和别的 `tui: null` 不一样
     * ═════════════════════════════════════════
     *
     * 别的那几条说的是「终端里做不了」（要浏览器、要认证器）。
     * 这一条是「还没做」—— 而且它是有意的：往终端里下载一个文件
     * 要先回答「存到哪」，那是一串新问题（当前目录？覆盖同名的？
     * 没有写权限时怎么办），而这些问题和「读一封信」完全无关。
     *
     * 端点已经上线，界面上也能下 —— 终端补这一屏时照着 api 那栏调即可。
     */
    tui: null,
    why: "往终端里下载文件要先回答「存到哪」——覆盖同名的怎么办、没写权限怎么办，那是一串和读信无关的新问题。端点已经在了，补的时候照着 api 那栏调",
    api: ["GET /api/v1/mail/attachments/{id}"],
    scopes: ["mail:burner"],
  },
  {
    key: "oauth.consent",
