 ...
 * ═════════════════════════════════════════
 *
 * 原来判据只有 `allow_claim` 那一个开关，而它在任何 kind 上都能是 1 ——
 * 于是线上三十九个 `owned`（**有主域名**）挂在了公共池里，
 * 站长自己的两个就在其中：任何人攒够分都能在他的域名上开地址。
 * 而 `openAlias` 那条路守得很严（「唯一的判据：这个域名是不是你的」）——
 * 两条路通向同一张表，只有一条设了闸。
 *
 * 写成白名单而不是「排除 owned」：以后加新的 kind 时，
 * 默认是**不进池子**，要进得有人明确加进来。
 * 反过来写的话，新 kind 会默认落进公共池 —— 而那个方向的错
 * 是「别人的域名被放上货架」。
 *
 * `temp`（一次性箱池）在里面是**有意的**：那些域名同时也接受长期申领，
 * 便宜档基本都在上面。代价是它们天天在发一次性地址，
 * 长期用的人要知道这一点 —— 那是取舍，不是疏漏。
 */
export const CLAIMABLE_DOMAIN_KINDS = ["reserved", "temp"] as const;
