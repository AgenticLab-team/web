...
import { parseTags, type TagIssue } from "@/lib/members/tags";
...
export interface SkillsResult {
  ...
  /*
 * `setDirectoryHidden` 删掉了。
 *
 * 隐身现在和榜单、检索那两个开关走同一条路
 * （`lib/privacy/actions.ts` 的 `setPrivacySwitch`）——
 * 三个问的是同一件事「谁看得见我」，而两条写同一列的路
 * 迟早会有一条忘了做另一条做的事。
 */
