 ...
 */
/**
 * ⚠️ 这个数**不再是门槛**，只在打分里当权重。
 *
 * 它曾经是「两条入场路」之一，而真实数据上那条路放进来的几乎全是
 * 噪音（详见 pickCatchphrases 里那段）。留着这个常量是因为
 * 打分那一行还在用它区分「同样够格的候选里谁更像一句话」。
 */
...
function rankCatchphrases(input: CatchphraseInput): Catchphrase[] {
  ...
  for (const [phrase, stat] of mine) {
     ...
     * ═════════════════════════════════════════
     *
     * 原来是「两个信号任一达标」。在真实数据上逐个人跟踪之后发现，
     * **边界那条路放进来的几乎全是噪音**：
     *
     *   进来的好东西  确实 67% / 牛逼 67% / 速吃 60% / 绷不住 40%
     *                等下 14% / 你看 11%     ← 整条率都不低
     *   进来的噪音    可以 0% / 直接 0% / 问题 0% / 注册 0% / 域名 0%
     *                不过 0% / 但是 0% / 重置 0% / 原神 0%
     *                ← **整条率清一色是 0**，全靠边界那条路进来
     *
     * 分得干净得不像话：好的没有一个低于 10%，噪音没有一个高于 0%。
     *
     * 而且这不只是第 2~5 名的问题 —— 有些人的**冠军**就是「可以」
     * 「不过」，同样是边界那条路放进来的。
     *
     * 边界率保留在打分里（同样够格时更像一句话的排前面），
     * 但它不再单独构成入场资格。
     *
     * 代价：有些人会一个口头禅都没有。那是对的 ——
     * 「这个人没有明显的口头禅」是一个真实的结论，
     * 而「他的口头禅是『不过』」是一句假话。
     */
    if (soloRate < MIN_SOLO_RATE) continue;
