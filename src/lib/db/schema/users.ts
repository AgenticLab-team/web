...
export const users = sqliteTable(
  "users",
  {
     ...
     * 留着 wx_id 的话，一个注销过的人的旧帖子照样显示他的昵称、
     * 头像，还带着能点进主页的链接 —— 「抹掉作者」就完全没有发生。
     *
     * 挡在读取侧（每处查询都判一下 status）是行不通的：
     * 论坛列表、收藏、积分后台、关注……漏一处就是一次泄露。
     * 断掉这条 join 是唯一一次做对、处处生效的办法。
     *
     * ─────────────────────────────────────────
     * 那为什么还要留一份
     * ─────────────────────────────────────────
     ...
     * 而注销重绑会拿到一个新的 user_id —— 规则自己的注释写着
     * 「不限制的话，注销重注册就能反复给同一个邀请人送奖励」，
     * 而注销这个功能恰恰把那条路打开了。
     *
     * 所以留一份**不参与任何展示查询**的副本，
     * 让那类一次性规则还认得出「这个微信号以前有过账号」。
     */
    priorWxId: text("prior_wx_id"),
