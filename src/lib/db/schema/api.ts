...
export const apiTokens = sqliteTable(
  "api_tokens",
  {
     ...
     * 到期时间；null = 不过期。
     *
     * 自己在页面上建的令牌是 null（长期有效，靠手动撤销）；
     * **OAuth 签出来的必须有期限** —— 那把钥匙在第三方手里，
     * 而用户忘了自己授权过什么是常态而不是例外。
     */
    expiresAt: integer("expires_at"),
    /**
     * 这把令牌是**哪个 OAuth 应用**换来的。自己建的是 null。
     *
     * ─────────────────────────────────────────
     * 加一列，而不是另起一套令牌体系
     * ─────────────────────────────────────────
     *
     * 这是整个 OAuth 设计里最要紧的一个决定。站里已经有一条完整的路：
     * `authenticate()` 验 scope、`sendAllowance()` 按人限流、
     * `recordSend()` 留痕、「我的 → 开放 API」能撤销和看日志。
     *
     * 另起一套要把这些每一条都重写一遍，而**重写出来的那份迟早
     * 少判一样东西** —— 这个仓库已经因为「同一件事两份实现」
     * 吃过好几次亏（发帖规则、分页边界、图标表、按钮长相）。
     *
     * 加了这一列之后，用户在同一个页面上看见「我自己建的」和
     * 「我授权的应用」，撤销走同一个按钮，代发日志里那一行本来就
     * 显示得出来源。全部免费。
     */
    appId: text("app_id"),
    /**
     ...
     * ═════════════════════════════════════════
     *
     * `ssh` 那一类的令牌明文躺在 SSH 网关那台机器上（见 `TUI.md` 第四节）。
     * 网关一旦被怀疑失守，要做的第一件事是把它签出的**全部**令牌作废 ——
     * 没有这一列的话，那个动作根本无从下手：库里只能看到一堆
     * 名字各异的令牌，分不出哪些在那台机器上。
     *
     * 顺带解决的是界面上的问题：「我自己建的」和「某台设备登录时拿的」
     * 混在一张列表里，人认不出哪一把该撤。
     */
    source: text("source").notNull().default("manual").$type<"manual" | "device" | "ssh">(),
    /**
     * 设备名，`source != 'manual'` 时有值。
     *
     * 和 `name` 分开存而不是拼进名字里：名字是人可以改的，
     * 而「这把是哪台机器上的」是事实，不该被改掉。
     */
