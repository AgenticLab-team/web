...
export const githubConnections = sqliteTable(
  "github_connections",
  {
     ...
     *
     * ─────────────────────────────────────────
     * 2026-08：默认从 false 改成 true
     * ─────────────────────────────────────────
     *
     * 原来的理由是「绑定 ≠ 同意公开」：有人绑定只是想要那个
     * 「有新项目要不要发帖」的提醒。这个顾虑本身没错，
     * 但站长定了另一个口径 —— 这是个技术社区，绑 GitHub 的人
     * 绝大多数**就是想让人看见**，而默认关着的结果是绑完之后
     * 主页上什么都没有，他会以为绑失败了。
     *
     * 保住那个顾虑的做法不是默认关，是**绑定时当场说清楚**：
     * 授权页上写明「绑定后你的公开仓库会显示在主页」，
     * 并且开关就在旁边，一键可关。知情 + 可退出 > 默认关闭 + 没人找得到。
     *
     * ⚠️ 只影响**新绑定**。已经绑过的人保持他们当时的选择 ——
     * 那是他们在「默认关」的规则下做的决定，回头替他们改成公开
     * 才是真的越界。
     */
    showOnProfile: integer("show_on_profile", { mode: "boolean" }).notNull().default(true),
     ...
     * ─────────────────────────────────────────
     *
     * 目录里每一行现在都是 GitHub 给的：名字、一句 description、
     * 语言、star 数。那些是**给陌生人看的**，而这个目录的读者
     * 是同一个群里的人 —— 他们想知道的是「这东西跟我有什么关系」
     * 「你为什么做它」「现在能用了吗」，而 README 的第一句话
     * 回答不了这些。
     *
     * 所以让作者自己写一句。它排在 GitHub 那句 description 前面，
     * 因为它是**这个人对这个社区说的话**。
     */
    pitch: text("pitch"),
    /** 自荐的是哪个仓库（full_name）。空 = 没自荐过 */
    pitchRepo: text("pitch_repo"),
    pitchAt: integer("pitch_at"),
