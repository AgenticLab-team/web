 ...
 * 派发状态全在内存里（每个用户攒几条、上次发在什么时候），
 * 两个进程各攒各的，于是**同一条通知会被推两遍**。
 *
 * 「通知会重复弹出」这件事这个站已经犯过一次了，那次是别的原因。
 *
 * ─────────────────────────────────────────
 * 为什么是租约，不是「让待机那边别跑」
 * ─────────────────────────────────────────
 *
 * 让待机那边别跑，就得让进程知道自己是不是待机的 ——
 ...
 * 部署、崩溃、手动重启，走的都是同一条路。
 */
export const processLeases = sqliteTable("process_leases", {
  /** 这是谁的租约 —— 目前只有 `notifications`，将来定时任务也能用 */
  name: text("name").primaryKey(),
  /** 持有者的进程标识。每个进程启动时随机生成，不复用 pid（pid 会被回收） */
