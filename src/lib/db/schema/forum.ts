...
export const NOTIFICATION_TYPES = [
  ...
  "moderation",
  /**
   * 站点公告、以及**你的发言被整理成帖子**这类和你本人有关的事。
   *
   * 这一类是关不掉的（见 prefs.ts 的 ALWAYS_ON）——
   * 所以它不能什么都往里装。称号解锁曾经也在这里：
   * 26 条通知里有 25 条是「解锁称号」，而它们和
   * 「你在群里的发言被整理成了帖子」共用一个关不掉的开关。
   *
   * 不关心徽章的人因此被迫收一串静不了音的消息 ——
   * 而由此产生的忽略，会连带把真正要紧的那一条一起淹掉。
   */
  "system",
  /** 解锁称号 —— 它是**可以关掉的**，见上面 system 那段 */
  "title",
  /** 关键词雷达命中 */
  "keyword",
] as const;
