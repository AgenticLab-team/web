...
export const mailBoxes = sqliteTable(
  "mail_boxes",
  {
    ...
    /**
     * 收到信转发到主人的私人邮箱（`users.email`）。**默认关。**
     *
     * ─────────────────────────────────────────
     * 开关挂在箱子上，地址挂在人身上
     * ─────────────────────────────────────────
     *
     * 一个人可能只想转发长期地址、不想转发一次性箱 ——
     * 后者一小时能收几十封验证码，全转到私人邮箱是灾难。
     * 所以开关是**每个箱子一个**。
     *
     * 而转发到哪，用的是 `users.email` 那个已经验证过的地址，
     * 不在这里再开一个字段：两个地址的话，「哪个是验证过的」
     * 会变成一个要查两处才能回答的问题，
     * 而答错的后果是把私信寄给一个没人验证过的地址。
     */
