...
import { Callout, Empty, PageNote, Pill, PillRow, SearchField } from "@/components/ui/primitives";
...
export default async function AdminPostsPage({
  ...
  return (
    <>
      ...
      {orphans.length > 0 && (
        <Callout tone="warning" title={`${orphans.length} 篇帖子的版块已被删除`}>
      ...
      <form action="/admin/posts" className="mb-3">
        ...
        <SearchField defaultValue={params.q} placeholder="搜标题或正文" />
      </form>
      ...
      <PillRow>
        ...
        {facets.boards.map((b) => (
          <Pill key={b.id} href={href({ board: b.id })} active={params.board === b.id}>
            {b.name} {b.count}
          </Pill>
        ))}
      </PillRow>
      ...
      <PageNote>
        批量操作一次最多 {BULK_LIMIT} 条，且<strong>每一条都会独立留处罚记录、独立通知作者</strong> ——
