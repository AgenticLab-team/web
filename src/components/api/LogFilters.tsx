...
export function LogFilters({
  ...
  return (
    <div className="inset-group mb-2 px-3.5 py-3">
      <div className="flex items-center gap-1.5 rounded-[var(--radius-control)] bg-[var(--surface-sunken)] px-2.5">
        <Search
        ...
        {text && (
          <button
            ...
            onClick={() => setText("")}
            aria-label="清空搜索"
            className="tap-target shrink-0 text-[var(--ink-quaternary)] transition active:opacity-60"
          >
      ...
      <div className="mt-2 flex flex-wrap gap-2">
        <select
          ...
          aria-label="按群筛选"
          className="t-footnote min-h-9 min-w-0 flex-1 rounded-[var(--radius-control)] bg-[var(--surface-sunken)] px-2.5 outline-none"
        >
        ...
        {showStatus && (
          <select
            value={status}
            onChange={(e) => apply({ status: e.target.value === "all" ? null : e.target.value })}
            aria-label="按成败筛选"
            className="t-footnote min-h-9 rounded-[var(--radius-control)] bg-[var(--surface-sunken)] px-2.5 outline-none"
          >
            <option value="all">成功和失败</option>
        ...
        {dirty && (
          <button
            ...
            className="t-footnote min-h-9 rounded-[var(--radius-control)] px-3 text-[var(--ink-secondary)] transition active:opacity-60"
