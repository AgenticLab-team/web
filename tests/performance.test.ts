...
describe("列表查询不能是 N+1", () => {
  ...
  it("**排行榜**：人数与天数再多也是一条聚合", () => {
    /*
     * 访客那条路比成员多一次查询：要知道榜上哪些人**注册过本站**，
     * 才能把没注册的那些隐去名字。
     *
     * 多的是**一次**，不是每行一次 —— 这条测试要守的正是这个区别。
     */
    assertNoNPlusOne(
      "排行榜（访客）",
      (n) => seedStats(n, 5),
      () => leaderboard.getLeaderboard({ convIds: ["g1"], viewer: null }),
      7,
    );
  });
  ...
  it("**排行榜（成员）**：同样是常数次", () => {
    /*
     * 和访客一样是 7 次，但那多出来的一次花在不同地方：
     * 访客花在「谁注册过本站」（匿名要用），
     * 成员花在权限判定（管理员看得到完整的榜）。
     *
     * 一开始我以为成员会少一次，写了个 6 的预算 —— 测试当场纠正了。
     * 预算写得比实际低不是「更严格」，是**下一个人改一行就得来猜数字**。
     */
    assertNoNPlusOne(
      "排行榜（成员）",
      (n) => seedStats(n, 5),
