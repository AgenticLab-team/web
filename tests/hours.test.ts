...
describe("**标签不能和它旁边那个时间段打架**", () => {
   ...
   *
   * 这一行真的出现在线上过 —— 分档按窗口**起点**算，
   * 于是 `from = 12` 落进「9–12 → 上午」，
   * 而 12:00–15:00 整个在下午。一句自相矛盾的话摆在同一行里。
   *
   * 上面那组测试挑的六个点（1/6/10/14/19/22）**恰好都不在边界上**，
   * 所以那个 bug 一直是绿的。这一组换个问法：
   * 遍历 24 个起点，逐个核对「标签说的那半天」真的包含这个窗口的重心。
   */
  /** 每个标签覆盖的钟点（按重心算，闭区间；跨零点的分成两段） */
  ...
  for (let from = 0; from < 24; from++) {
    it(`${from}:00 起的那三小时，标签对得上`, () => {
      const h = hist({ [from]: 60, [(from + 1) % 24]: 60, [(from + 2) % 24]: 60 });
      const s = summarizeHours(h)!;
      assert.ok(s.label, `${from} 点没给出标签`);

      const ranges = RANGES[s.label!];
      assert.ok(ranges, `冒出一个没登记的标签：${s.label}`);

      // 窗口的重心 —— 三小时的中间那个钟点
      const mid = (from + 1) % 24;
      const covered = ranges.some(([a, b]) => mid >= a && mid <= b);
      assert.ok(
        covered,
        `窗口 ${formatWindow(from, (from + 2) % 24)} 的重心在 ${mid} 点，` +
          `而标签说的是「${s.label}」—— 这两句会挨着显示在同一行`,
      );
    });
  }
});
